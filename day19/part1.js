...
fs.readFile('./input', 'utf-8', (_, data) => {
  ...
  blueprints.forEach(blueprint => {
    step(blueprint, 1, 0, 0, 0, 0, 0, 0, 0, 0);
  });
  let totalQuality = 0;
  blueprints.forEach(blueprint => {
    blueprint.quality = blueprint.id * blueprint.answer;
    totalQuality += blueprint.quality;
  });
  console.log(`Solution is ${totalQuality}`);
});
